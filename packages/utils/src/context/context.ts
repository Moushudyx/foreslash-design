 ...
 */
export const _createContext = <ValueType>(key: unknown) => key as Context<typeof key, ValueType>;
...
export class ContextRequestEvent<T extends UnknownContext> extends Event {
  public constructor(
    ...
    public readonly resubscribe?: boolean
  ) {
 ...
 * 一个上下文取消订阅事件
 */
export class ContextUnsubscribeEvent<T extends UnknownContext> extends Event {
  public constructor(
    public readonly context: T,
    public readonly targetElement: HTMLElement,
  ) {
    super('context-unsubscribe', { bubbles: true, composed: true });
    this.context = context;
    this.targetElement = targetElement;
  }
}
...
declare global {
  interface HTMLElementEventMap {
    ...
    'context-unsubscribe': ContextUnsubscribeEvent<Context<unknown, unknown>>;
  }
}
